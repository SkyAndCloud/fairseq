...
CUDA_VISIBLE_DEVICES=0 python train.py /home/shanyong/wmt16_en_ro/fairseq_databin \
    --ddp-backend no_c10d \
    --arch abdnmt_base_nist --bd-write --agree-weight 0 --share-all-embeddings \
    ...
    --max-epoch 30 \
    --update-freq 4 --no-progress-bar --log-format json --log-interval 100 | tee -a logs/$model.log
